...
# merge할 대상 컬럼 추가
sh_title_df['title_join'] = sh_title_df['title']
...
# 타이틀명 변경
merged_df.rename(columns={"title":"merged_title"}, inplace=True)

# DataWarehouse에 수집된 스팀 게임 정보 불러오기
...
model_action_df = find_genre[find_genre['genre'].str.contains('Action')]
model_indie_df = find_genre[find_genre['genre'].str.contains('Indie')]
model_adventure_df = find_genre[find_genre['genre'].str.contains('Adventure')]
model_casual_df = find_genre[find_genre['genre'].str.contains('Casual')]
model_simulation_df = find_genre[find_genre['genre'].str.contains('Simulation')]
model_strategy_df = find_genre[find_genre['genre'].str.contains('Strategy')]
model_rpg_df = find_genre[find_genre['genre'].str.contains('RPG')]
model_sports_df = find_genre[find_genre['genre'].str.contains('Sports')]
model_racing_df = find_genre[find_genre['genre'].str.contains('Racing')]

# 7월 21일 기준 장르별 상위 데이터 추가 수집하여 불러오기
